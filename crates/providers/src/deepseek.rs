...
impl LLMProvider for DeepSeekProvider {
    ...
    async fn send_request(&self, request: Request) -> securellm_core::Result<Response> {
        ...
        let securellm_response = self
            .convert_response(request.id, deepseek_response, processing_time)
            .map_err(|e| Error::Provider {
                provider: "deepseek".to_string(),
    ...
    async fn stream_request(&self, mut request: Request) -> securellm_core::Result<ProviderStream> {
        ...
        crate::openai_compat_stream::send_stream("deepseek", &request, req_builder).await
    }
    ...
    async fn list_models(&self) -> securellm_core::Result<Vec<ModelInfo>> {
        ...
        Ok(vec![
            ModelInfo {
                ...
                description: Some(
                    "DeepSeek's chat model with strong reasoning capabilities".to_string(),
                ),
            ...
            ModelInfo {
                ...
                description: Some(
                    "Specialized model for code generation and understanding".to_string(),
                ),
                context_window: Some(64000),
                max_output_tokens: Some(4096),
